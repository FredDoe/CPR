...
def generate_special_paths(ppc_list, arg_list, poc_path, bin_path):
    ...
    if values.DEFAULT_OPERATION_MODE in ["sequential", "semi-parallel"]:
        ...
        for con_loc, ppc_str in ppc_list[: values.DEFAULT_MAX_FLIPPINGS]:
            ...
            count = count + 1
            pool.apply_async(
                generator.generate_special_paths,
    ...
    if values.DEFAULT_OPERATION_MODE in ["sequential", "semi-parallel"]:
        for control_loc, ppc in ppc_list[: values.DEFAULT_MAX_FLIPPINGS]:
            ...
            if new_path_str not in values.LIST_PATH_CHECK:
                ...
                result_list.append(
                    oracle.check_path_feasibility(control_loc, new_path, count - 1)
                )
        ...
        for control_loc, ppc in ppc_list[: values.DEFAULT_MAX_FLIPPINGS]:
            ...
            if new_path_str not in values.LIST_PATH_CHECK:
                ...
                abortable_func = partial(
                    abortable_worker,
                    oracle.check_path_feasibility,
                    default=False,
                    index=count - 1,
                )
                pool.apply_async(
                    abortable_func,
                    args=(control_loc, new_path, count - 1),
...
def validate_patches_parallel(patch_list, path_condition, assertion):
    ...
    if values.DEFAULT_OPERATION_MODE in ["sequential"]:
        for patch in patch_list:
            ...
            result_list.append(
                oracle.check_patch_feasibility(
                    assertion,
                    var_relationship,
                    patch_formula_extended,
                    path_condition,
                    index,
                )
            )
        ...
        if is_sat(is_exist):
            ...
            if values.DEFAULT_OPERATION_MODE in ["sequential"]:
                ...
                for partition in partition_list:
                    pool.apply_async(
                        refine.refine_input_partition,
                        args=(path_condition, assertion, partition, is_multi_dimension),
                        callback=collect_result,
                    )
                pool.close()
                emitter.normal("\t\twaiting for thread completion")
        ...
        for patch in patch_list:
            ...
            if values.DEFAULT_PATCH_TYPE == values.OPTIONS_PATCH_TYPE[1]:
                ...
                if parameter_constraint:
                    patch_space_constraint = And(
                        patch_formula_extended, parameter_constraint
                    )
            index = list(patch_list).index(patch)
            # emitter.emit_patch(patch, message="\tabstract patch " + str(index) + " :")
            result_list.append(
                oracle.check_input_feasibility(index, patch_space_constraint, new_path)
            )
    else:
        emitter.normal("\t\tstarting parallel computing")
        pool = mp.Pool(mp.cpu_count(), initializer=mute)
        ...
        for patch in patch_list:
            try:
                ...
                thread = pool.apply_async(
                    oracle.check_input_feasibility,
                    args=(index, patch_space_constraint, new_path),
