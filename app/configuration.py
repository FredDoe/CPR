        ...
        for arg in arg_list:
            if definitions.ARG_DEBUG in arg:
                ...
                values.CONF_RANK_LIMIT = int(
                    arg.replace(definitions.ARG_RANK_LIMIT, "")
                )
                ...
                if option not in values.OPTIONS_SELECT_METHOD:
                    emitter.error(
                        "Invalid option for "
                        + definitions.ARG_SELECTION_METHOD.replace("=", "")
                        + " : "
                        + arg
                    )
                    emitter.emit_help()
                ...
                if option not in values.OPTIONS_OPERATION_MODE:
                    emitter.error(
                        "Invalid option for "
                        + definitions.ARG_OPERATION_MODE.replace("=", "")
                        + " : "
                        + arg
                    )
                    emitter.emit_help()
                ...
                if option not in values.OPTIONS_PATCH_TYPE:
                    emitter.error(
                        "Invalid option for "
                        + definitions.ARG_PATCH_TYPE.replace("=", "")
                        + " : "
                        + arg
                    )
                    emitter.emit_help()
                ...
                if option not in values.OPTIONS_REFINE_METHOD:
                    emitter.error(
                        "Invalid option for "
                        + definitions.ARG_REFINE_METHOD.replace("=", "")
                        + " : "
                        + arg
                    )
                    emitter.emit_help()
                ...
                if option not in values.OPTIONS_REDUCE_METHOD:
                    emitter.error(
                        "Invalid option for "
                        + definitions.ARG_REDUCE_METHOD.replace("=", "")
                        + " : "
                        + arg
                    )
                    emitter.emit_help()
                ...
                values.CONF_TEST_SUITE_ID_LIST = (
                    str(arg).replace(definitions.ARG_TEST_SUITE_ID_LIST, "").split(",")
                )
            elif definitions.ARG_SEED_SUITE_ID_LIST in arg:
                values.CONF_SEED_SUITE_ID_LIST = (
                    str(arg).replace(definitions.ARG_SEED_SUITE_ID_LIST, "").split(",")
                )
            elif definitions.ARG_ITERATION_COUNT in arg:
                values.CONF_ITERATION_LIMIT = int(
                    arg.replace(definitions.ARG_ITERATION_COUNT, "")
                )
                ...
                values.CONF_TIME_DURATION = float(
                    arg.replace(definitions.ARG_TIME_DURATION, "")
                )
                ...
                if ":" not in arg:
                    emitter.error(
                        "Invalid option for "
                        + definitions.ARG_CEGIS_TIME_SPLIT.replace("=", "")
                        + " : "
                        + arg
                    )
                    emitter.emit_help()
...
def read_conf_file():
    ...
    for configuration in configuration_list:
        if definitions.CONF_PATH_PROJECT in configuration:
            ...
            values.CONF_GENERALIZED_SEED_INPUT = str(
                configuration.replace(definitions.CONF_GENERALIZED_SEED_INPUT, "")
            )
        elif definitions.CONF_GENERALIZED_TEST_INPUT in configuration:
            values.CONF_GENERALIZED_TEST_INPUT = str(
                configuration.replace(definitions.CONF_GENERALIZED_TEST_INPUT, "")
            )
            ...
            assertion_file_path = (
                values.CONF_PATH_PROJECT + "/" + values.CONF_PATH_SPECIFICATION
            )
            ...
            if values.CONF_SELECTION_STRATEGY not in values.OPTIONS_SELECT_METHOD:
                error_exit(
                    "Invalid configuration for " + definitions.CONF_SELECTION_STRATEGY
                )
            ...
            if values.CONF_DISTANCE_METRIC not in values.OPTIONS_DIST_METRIC.values():
                error_exit(
                    "Invalid configuration for " + definitions.CONF_DISTANCE_METRIC
                )
            ...
            if values.CONF_OPERATION_MODE not in values.OPTIONS_OPERATION_MODE:
                error_exit(
                    "Invalid configuration for " + definitions.CONF_OPERATION_MODE
                )
            ...
            values.CONF_MAX_FLIPPINGS = int(
                configuration.replace(definitions.CONF_MAX_FLIPPINGS, "")
            )
            ...
            values.CONF_TIMEOUT_CONCOLIC_RUN = int(
                configuration.replace(definitions.CONF_TIMEOUT_CONCOLIC_RUN, "")
            )
        elif definitions.CONF_TIMEOUT_CONCRETE_RUN in configuration:
            values.CONF_TIMEOUT_CONCRETE_RUN = int(
                configuration.replace(definitions.CONF_TIMEOUT_CONCRETE_RUN, "")
            )
        elif definitions.CONF_TEST_SUITE_ID_LIST in configuration:
            values.CONF_TEST_SUITE_ID_LIST = (
                str(configuration)
                .replace(definitions.CONF_TEST_SUITE_ID_LIST, "")
                .split(",")
            )
        elif definitions.CONF_SEED_SUITE_ID_LIST in configuration:
            values.CONF_SEED_SUITE_ID_LIST = (
                str(configuration)
                .replace(definitions.CONF_SEED_SUITE_ID_LIST, "")
                .split(",")
            )
            ...
            if os.path.isfile(config_path):
                ...
                error_exit(
                    "Test suite configuration file not found at " + str(config_path)
                )
            ...
            if os.path.isfile(config_path):
                ...
                error_exit(
                    "Seed suite configuration file not found at " + str(config_path)
                )
        elif definitions.CONF_TEST_BINARY_CONFIG_FILE in configuration:
            config_path = configuration.replace(
                definitions.CONF_TEST_BINARY_CONFIG_FILE, ""
            )
            config_path = values.CONF_PATH_PROJECT + "/" + config_path
            if os.path.isfile(config_path):
                with open(config_path, "r") as conf_file:
                    values.CONF_TEST_BINARY_LIST = [
                        x.strip().replace("\n", "") for x in conf_file.readlines()
                    ]
            else:
                error_exit(
                    "Test binary configuration file not found at " + str(config_path)
                )
        elif definitions.CONF_SEED_BINARY_CONFIG_FILE in configuration:
            config_path = configuration.replace(
                definitions.CONF_SEED_BINARY_CONFIG_FILE, ""
            )
            config_path = values.CONF_PATH_PROJECT + "/" + config_path
            if os.path.isfile(config_path):
                with open(config_path, "r") as conf_file:
                    values.CONF_SEED_BINARY_LIST = [
                        x.strip().replace("\n", "") for x in conf_file.readlines()
                    ]
            else:
                error_exit(
    ...
    if values.CONF_PATH_PROGRAM:
        if "/" != values.CONF_PATH_PROGRAM[0]:
            values.CONF_PATH_PROGRAM = (
                values.CONF_DIR_SRC + "/" + values.CONF_PATH_PROGRAM
            )
...
def collect_test_list():
    ...
    if values.CONF_TEST_SUITE_CONFIG:
        for (
            ...
            test_input,
            expected_output,
        ) in values.CONF_TEST_SUITE_CONFIG:
                ...
                for content in content_lines:
                    ...
                    if "$POC_" in test_input:
                        ...
                        if values.CONF_GENERALIZED_TEST_INPUT:
                            test_input = values.CONF_GENERALIZED_TEST_INPUT.replace(
                                "$POC", test_input
                            )
                        if test_input_file[0] != "/":
                            if values.CONF_SEED_DIR:
                                test_input_file = (
                                    values.CONF_SEED_DIR + "/" + test_input_file
                                )
                            else:
                                test_input_file = (
                                    values.CONF_DIR_SRC + "/" + test_input_file
                                )
...
def collect_seed_list():
                ...
                for content in content_lines:
                    ...
                    if "$POC_" in seed_input:
                        ...
                        if values.CONF_GENERALIZED_SEED_INPUT:
                            seed_input = values.CONF_GENERALIZED_SEED_INPUT.replace(
                                "$POC", seed_input
                            )
                        if seed_input_file[0] != "/":
                            if values.CONF_SEED_DIR:
                                seed_input_file = (
                                    values.CONF_SEED_DIR + "/" + seed_input_file
                                )
                            else:
                                seed_input_file = (
                                    values.CONF_DIR_SRC + "/" + seed_input_file
                                )
    ...
    if values.CONF_SEED_DIR:
        ...
        file_list = [
            f for f in os.listdir(seed_dir) if os.path.isfile(os.path.join(seed_dir, f))
        ]
...
def collect_var_mapping():
    ...
    with open(source_file_path, "r") as source_file:
        ...
        for i in range(0, len(prog_var_list)):
            values.MAP_PROG_VAR[
                comp_name_list[i].strip().replace('"', "").replace("'", "")
            ] = prog_var_list[i]
...
def update_configuration():
    ...
    definitions.DIRECTORY_OUTPUT = (
        definitions.DIRECTORY_OUTPUT_BASE + "/" + values.CONF_TAG_ID
    )
    definitions.DIRECTORY_LOG = (
        definitions.DIRECTORY_LOG_BASE + "/" + values.CONF_TAG_ID
    )
    ...
    if values.CONF_TIME_SPLIT:
        ...
        values.CONF_TIME_CEGIS_EXPLORE = (
            int(explore) / total
        ) * values.DEFAULT_TIME_DURATION
        values.CONF_TIME_CEGIS_REFINE = (
            int(refine) / total
        ) * values.DEFAULT_TIME_DURATION
