...
class Instance(Enum):
    ...
    def of_symbol(symbol: Formula, parameter: Union[int, str], inst) -> Formula:
        prefix = {
            Instance.EXECUTION: f"exe:{parameter}!",
    ...
    @staticmethod
    def of_formula(
        formula: Formula, parameter: Union[int, str], inst, predicate
    ) -> Formula:
        return formula.substitute(
            {
                s: Instance.of_symbol(s, parameter, inst)
                for s in collect_symbols(formula, predicate)
            }
        )
...
class TridentType(Enum):
    ...
    def __str__(self):
        return {TridentType.BOOL: "bool", TridentType.I32: "i32"}[self]

    @staticmethod
    def parse(s):
        return {"bool": TridentType.BOOL, "i32": TridentType.I32}[s]
        ...
        rholes = list(
            collect_symbols(
                ...
                all_fn(
                    ...
                    lambda x: ComponentSymbol.parse(x).name == name,
                ),
            )
        )
...
def program_to_formula(program: Program) -> Formula:
    ...
    def tree_to_formula(tree, node_counter):
        ...
        root_rbranch = None
        if rreturn:
            root_rbranch = Instance.of_symbol(
                ComponentSymbol.branch_of(rreturn), nid, Instance.NODE
            )
            branch_substitution[rreturn] = root_rbranch
        lreturn = ComponentSemantics.get_lreturn(root)
        root_lbranch = None
        if lreturn:
            root_lbranch = Instance.of_symbol(
                ComponentSymbol.branch_of(lreturn), nid, Instance.NODE
            )
        ...
        instantiated_root = Instance.of_formula(
            branch_root, nid, Instance.NODE, is_return
        )
        hole_substitution = {}
        children_semantics = TRUE()
        for hole_id, subtree in children.items():
            (subtree_semantics, subtree_rbranch, subtree_lbranch) = tree_to_formula(
                subtree, node_counter
            )
            if subtree_rbranch:
    ...
    const_substitution = {
        ComponentSymbol.const(k): SBV(v, 32) for k, v in constants.items()
    }
...
def program_to_code(program: Program) -> str:
    ...
    def tree_to_code(tree):
        ...
        if m and m.group(1) in constants:
            ...
            if (
                set(children.keys()) == set(["left", "right"])
                and cid in binary_operators.keys()
            ):
                return f"({tree_to_code(children['left'])} {binary_operators[cid]} {tree_to_code(children['right'])})"
            elif (
                set(children.keys()) == set(["argument"])
                and cid in unary_operators.keys()
            ):
                ...
                set(children.keys()) == set(["condition", "left", "right"])
                and cid == "guarded_assignment"
            ):
    ...
    def tree_to_json(tree):
        ...
        for k, v in children.items():
            symbols = collect_symbols(
                semantics, lambda s: ComponentSymbol.parse(s).name == k
            )
            if any(
                any_fn(ComponentSymbol.is_rhole, ComponentSymbol.is_lhole)(s)
                for s in symbols
            ):
...
class RuntimeSymbol(Enum):
    ...
    def parse(symbol: Formula) -> SymbolData:
        ...
        if kind == RuntimeSymbol.ANGELIC:
            return SymbolData(
                type=TridentType.parse(m.group(1)), lid=m.group(2), eid=int(m.group(3))
            )
            ...
            return SymbolData(
                type=TridentType.parse(m.group(1)), name=m.group(2), eid=int(m.group(3))
            )
        else:
...
class Klee:
    ...
    def interpret_memory(array: Formula, type: TridentType) -> Formula:
        concat = BVConcat(
            Select(array, BV(3, 32)),
            BVConcat(
                Select(array, BV(2, 32)),
    ...
    def load(tid: str, assertion: Formula, klee_dir: Path) -> Specification:
        ...
        for pathfile in smt2s:
            ...
            with pathfile.open() as f:
                ...
                test_instantiated = Instance.of_formula(
                    path_formula, tid, Instance.TEST, RuntimeSymbol.is_nonspecial
                )
                path_instantiated = Instance.of_formula(
                    test_instantiated, pid, Instance.PATH, RuntimeSymbol.is_nonspecial
                )
                pysmt.environment.pop_env()
                normalized = pysmt.environment.get_env().formula_manager.normalize(
                    path_instantiated
                )
...
def extract_lids(path: Formula) -> Dict[str, TridentType]:
    ...
    lids = {
        RuntimeSymbol.parse(s).lid: RuntimeSymbol.parse(s).type for s in angelic_symbols
    }
    ...
    lids.update(
        {
            RuntimeSymbol.parse(s).lid: TridentType.I32
            for s in lvalue_symbols
...
def extract_eids(path: Formula, lid: str) -> Set[str]:
    ...
    symbols = collect_symbols(
        path, any_fn(RuntimeSymbol.is_angelic, RuntimeSymbol.is_lvalue)
    )
    return set(
        RuntimeSymbol.parse(s).eid for s in symbols if RuntimeSymbol.parse(s).lid == lid
    )
...
def verify(
    programs: Union[Dict[str, Program], Dict[str, Formula]],
    ...
    program_semantics = {
        lid: (program if isinstance(program, Formula) else program_to_formula(program))
        for (lid, program) in programs.items()
    }
    free_variables = [
        v
        for p in program_semantics.values()
        for v in collect_symbols(p, ComponentSymbol.is_const)
    ]
    ...
    for tid in specification.keys():
        ...
        for path in paths:
            ...
            for eid in eids:
                ...
                for program_symbol in collect_symbols(program, lambda x: True):
                    ...
                    data = (
                        ComponentSymbol.parse(program_symbol)
                        ._replace(lid=lid)
                        ._replace(eid=eid)
                    )
                    if kind == ComponentSymbol.RRETURN:
                        program_substitution[program_symbol] = RuntimeSymbol.angelic(
                            data
                        )
                    elif kind == ComponentSymbol.RVALUE:
                        program_substitution[program_symbol] = RuntimeSymbol.rvalue(
                            data
                        )
                    elif kind == ComponentSymbol.LVALUE:
                        program_substitution[program_symbol] = RuntimeSymbol.lvalue(
                            data
                        )
                    else:
                ...
                is_branch = any_fn(
                    ComponentSymbol.is_rbranch, ComponentSymbol.is_lbranch
                )
                exe_inst_program = Instance.of_formula(
                    bound_program, eid, Instance.EXECUTION, is_branch
                )
        ...
        is_special_nonconst = any_fn(
            RuntimeSymbol.is_special,
            all_fn(ComponentSymbol.is_special, complement(ComponentSymbol.is_const)),
        )
        instantiated_test_vc = Instance.of_formula(
            test_vc, tid, Instance.TEST, is_special_nonconst
        )
    ...
    if model is None:
        ...
        return VerificationSuccess(
            {v: model[v].bv_signed_value() for v in free_variables}
        )
...
def enumerate_trees(
    ...
    roots = [
        ...
        if (
            not need_lreturn
            or (
                ComponentSemantics.get_lreturn(c[1])
                and ComponentSymbol.parse(ComponentSemantics.get_lreturn(c[1])).type
                == typ
            )
        )
        and (
            not need_rreturn
            or (
                ComponentSemantics.get_rreturn(c[1])
                and ComponentSymbol.parse(ComponentSemantics.get_rreturn(c[1])).type
                == typ
            )
        )
    ]
    ...
    def enumerate_mappings(names, mapping_params, drop):
        ...
        for i, substitution in enumerate(subtrees):
            if remaining:
                ...
                for partial_mapping in enumerate_mappings(
                    remaining, mapping_params, next_drop
                ):
    ...
    for root in roots:
        holes = collect_symbols(
            root[1], any_fn(ComponentSymbol.is_lhole, ComponentSymbol.is_rhole)
        )
        if not holes:
            yield (root, {})
        if holes and (depth > 1):
            names = list(set(ComponentSymbol.parse(h).name for h in holes))
            commutative = root[0] in [
                "addition",
                "multiplication",
                "equal",
                "not-equal",
            ...
            for name in names:
                ...
                typ = (
                    ComponentSymbol.parse(lhole).type
                    if lhole
                    else ComponentSymbol.parse(rhole).type
                )
...
def extract_assigned(tree: ComponentTree) -> List[Component]:
    (root, mapping) = tree
    assigned_names = (
        name for name in mapping.keys() if ComponentSemantics.get_lhole(root[1], name)
    )
    ...
    for tree in enumerate_trees(components, depth, typ, False, True):
        ...
        if concrete_enumeration and (contains_constant(tree[1]) or not optimized):
                    ...
                    if not is_tautology_included():
                        if is_always_tautology:
                            ...
                            pool.apply_async(
                                check_sat,
                                args=(vc, index),
                                callback=collect_tautology_result,
                            )
                            continue
                        else:
                    ...
                    if not is_contradiction_included():
                        if is_always_contradiction:
                            ...
                            pool.apply_async(
                                check_sat,
                                args=(vc, index),
                                callback=collect_contradiction_result,
                            )
                            continue
                        else:
            ...
            # There is at least one variable and one constant, then iterate over possible constant values and verify the concrete patch.
                ...
                if (
                    not optimized and len(names) == 0
                ):  # TODO remove len=0, just for testing
                    for value_a in range(lower_bound, upper_bound):
                        ...
                        try:
                            pool.apply_async(
                                check_sat, args=(vc, index), callback=collect_one
                            )
                        except ValueError:
                            break
                elif len(names) == 1:
                    ...
                    for value_a in range(lower_bound, upper_bound):
                        ...
                        for value_b in range(lower_bound, upper_bound):
                            patch = {
                                lid: (
                                    tree,
                                    {const_a_str: value_a, const_b_str: value_b},
                                )
                            }
                            index = atomic_patchlist_append(patch)
                            vc = verify_parallel(patch, specification)
        ...
        if concrete_enumeration and (contains_constant(tree[1]) or not optimized):
            ...
            # There is at least one variable and one constant, then iterate over possible constant values and verify the concrete patch.
                ...
                if (
                    ...
                    for value_a in range(lower_bound, upper_bound):
                        result = verify(
                            {lid: (tree, {const_a_str: value_a})}, specification
                        )
                        if result:
            ...
            if result:
                yield {
                    lid: (
                        tree,
                        {
                            ComponentSymbol.parse(f).name: v
...
def synthesize_parallel(
    ...
    for tree in enumerate_trees(components, depth, typ, False, True):
                ...
                if (
                    ...
                    for value_a in range(lower_bound, upper_bound):
                        ...
                        pool.apply_async(
                            check_sat,
                            args=(vc, index),
                            callback=collect_synthesis_result,
                        )
                elif len(names) == 1:
                    const_a_str = names.pop()
                    for value_a in range(lower_bound, upper_bound):
        ...
        ## Otherwise: verify the patch and report it.
            ...
            vc = verify_parallel(patch, specification)
            pool.apply_async(
                check_sat, args=(vc, index), callback=collect_synthesis_result
            )
...
def contains_constant(tree_content: Dict[str, "ComponentTree"]) -> bool:
    ...
    left_dict = tree_content["left"][
        1
    ]  # if there is a non-empty dict, then left is no leaf node
    right_name = tree_content["right"][0][0]
    right_dict = tree_content["right"][
        1
    ]  # if there is a non-empty dict, then right is no leaf node
...
def load_programs(
    files: Dict[str, Path], components: List[Component]
) -> Dict[str, Union[Program, Formula]]:
...
def load_components(comp_files: List[Path]) -> List[Component]:
    ...
    for component_file in comp_files:
        ...
        with component_file.open() as f:
            ...
            is_var_or_const = any_fn(
                ComponentSymbol.is_rvalue,
                ComponentSymbol.is_lvalue,
                ComponentSymbol.is_const,
            )
            # we assume that variables and constants are global, but everything else is local
            instantiated = Instance.of_formula(
                semantics, cid, Instance.COMPONENT, complement(is_var_or_const)
            )
            pysmt.environment.pop_env()
            normalized = pysmt.environment.get_env().formula_manager.normalize(
                instantiated
