...
import app.parallel
from app.concolic import (
    run_concolic_execution,
    select_new_input,
    check_infeasible_paths,
    run_concrete_execution,
)
...
def update_index(result_list, patch_list_new, patch_list_old):
    ...
    for result in result_list:
        ...
        updated_list.append(
            (refined_space, new_index, patch_score, is_under_approx, is_over_approx)
        )
    return updated_list
...
def recover_patch_list(result_list, patch_list, path_condition, assertion):
    ...
    if len(diff_list_a) != len(result_list_a):
        ...
        result_list_b = parallel.refine_patch_space(
            diff_list_b, path_condition, assertion, True
        )
        recover_list = recover_list + update_index(
            result_list_b, diff_list_b, patch_list
        )
    return recover_list
...
def update_patch_list(result_list, patch_list, path_condition, assertion):
    ...
    if len(result_list) != len(patch_list):
        recover_list = recover_patch_list(
            result_list, patch_list, path_condition, assertion
        )
...
def reduce(
    patch_list, path_to_concolic_exec_result, assertion
) -> List[Tuple[str, Program]]:  # TODO
    ...
    updated_patch_list = update_patch_list(
        result_list, patch_list, path_condition, assertion
    )
...
def print_patch_list(patch_list):
    template_count = 0
    emitter.sub_title(
        "List of Top " + str(values.DEFAULT_PATCH_RANK_LIMIT) + " Correct Patches"
    )
    if not patch_list:
        ...
        if values.DEFAULT_PATCH_TYPE == values.OPTIONS_PATCH_TYPE[1]:
            ...
            for partition in patch_space:
                ...
                for constant_name in partition:
                    ...
                    concrete_patch_count = (
                        utilities.count_concrete_patches_per_template(patch)
                    )
...
def run_cegis(program_path, project_path, patch_list):
    ...
    while not satisfied:
        ...
        if is_sat(violation_check):
            ...
            values.FILE_POC_GEN = (
                definitions.DIRECTORY_OUTPUT + "/violation-" + str(values.ITERATION_NO)
            )
...
def compute_iteration_stat(patch_list, iteration):
    ...
    definitions.FILE_PATCH_SET = (
        definitions.DIRECTORY_OUTPUT + "/patch-set-ranked-" + str(iteration)
    )
...
def run_cpr(program_path, patch_list):
    ...
    while not satisfied and len(patch_list) > 0:
        if iteration == 0:
            ...
            for argument_list in test_input_list:
                ...
                klee_out_dir = (
                    output_dir_path + "/klee-out-repair-" + str(iteration - 1)
                )
                ...
                for arg in argument_list:
                    if str(argument_list.index(arg)) in values.CONF_MASK_ARG:
                        ...
                        list(values.LIST_SEED_FILES.values())
                ...
                emitter.sub_sub_title(
                    "Iteration: " + str(iteration) + " - Using Seed #" + str(seed_id)
                )
                ...
                if seed_id > count_fail_inputs:
                    exit_code = run_concrete_execution(
                        program_path + ".bc", argument_list, True, klee_test_dir
                    )
                    assert exit_code == 0
                    # set location of bug/crash
                    values.IS_CRASH = False
                    latest_crash_loc = reader.collect_crash_point(
                        values.FILE_MESSAGE_LOG
                    )
                    ...
                    if latest_crash_loc:
                        ...
                        emitter.success(
                            "\t\t\t[info] identified a crash location: "
                ...
                _, second_var_list = generator.generate_angelic_val(
                    klee_test_dir, generalized_arg_list, poc_path
                )
                ...
                generated_path_list = app.parallel.generate_symbolic_paths(
                    values.LIST_PPC, generalized_arg_list, poc_path, program_path
                )
                ...
                assertion, count_obs = generator.generate_assertion(
                    assertion_template, Path(klee_out_dir).resolve()
                )
            ...
            exit_code = run_concolic_execution(
                program_path + ".bc", gen_arg_list, gen_var_list, False, klee_out_dir
            )
            # assert exit_code == 0
            duration = (time.time() - time_check) / 60
            values.COUNT_PATHS_EXPLORED = values.COUNT_PATHS_EXPLORED + 1
            ...
            duration = (time.time() - time_check) / 60
            emitter.note(
                "\t\treduce finished in " + str(format(duration, ".3f")) + " minutes"
            )
            values.TIME_TO_REDUCE = values.TIME_TO_REDUCE + duration
            if satisfied:
                emitter.warning(
    ...
    if not patch_list:
        ...
        definitions.FILE_PATCH_SET = (
            definitions.DIRECTORY_OUTPUT + "/patch-set-original"
        )
        writer.write_patch_set(patch_list, definitions.FILE_PATCH_SET)
        ranked_patch_list = rank_patches(patch_list)
